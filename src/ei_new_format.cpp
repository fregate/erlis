...
static int pmap(const char ** fmt, union arg **, ei_x_buff * x, int size, bool key);
...
static int eiformat(const char ** fmt, union arg ** args, ei_x_buff * x)
{
	...
	case '#':
		if (*(p + 1) == '{')
		{
			res = ei_x_new(&x2);
			if (res >= 0)
				res = pmap(&++p, args, &x2, 0, true);
			if (res >= 0 && res % 2 == 0)
				res = ei_x_encode_map_header(x, res / 2);
			else
				res = -1;
			if (res >= 0)
				res = ei_x_append(x, &x2);
...
/* encode a map */
static int pmap(const char ** fmt, union arg ** args, ei_x_buff * x, int size, bool key)
{
	int res = 0;
	const char * p = *fmt;
	char after = *p++;
	...
	case '>':
		...
		break;
	default:
		if (after == '>' || after == '{' || (after == ',' && key))
		{
			--p;
			res = eiformat(&p, args, x);
			if (res >= 0)
				res = pmap(&p, args, x, size + 1, !key);
		}
		else
			res = -1;
		break;
...
int ei_new_format(ei_x_buff * x, const char * fmt, ...)
{
	va_list ap;
	union arg * args;
...
int ei_new_format_wo_ver(ei_x_buff * x, const char * fmt, ...)
{
	va_list ap;
	union arg * args;
	union arg * saved_args;
